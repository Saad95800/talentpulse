 ...
 */
function FormattedText({ text }: { text: string | unknown }) {
        ...
        <div className="flex-1 overflow-y-auto p-10 custom-scrollbar">
          {type === 'job' ? (
            <div className="max-w-none">
               <FormattedText text={typeof data === 'string' ? data : (data as InfoData).description || ''} />
            </div>
          ) : (
                ...
                return (
                  <>
                      ...
                      <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                        <div>
                          ...
                          <h4 className="text-4xl font-black tracking-tight flex gap-3">
                            ...
                            <span className="text-primary">{d.lastName || ''}</span>
                          </h4>
                          {d.position && <p className="text-slate-400 font-bold mt-2">{d.position}</p>}
                        </div>
                        <div className="flex flex-wrap gap-3">
                           {d.email && (
                             <div className="flex items-center gap-2 px-4 py-2 bg-white/10 backdrop-blur-sm rounded-xl border border-white/10 text-xs font-bold">
                               <Mail className="w-4 h-4 text-primary" /> {d.email}
                             </div>
                           )}
                           {d.phone && (
                             <div className="flex items-center gap-2 px-4 py-2 bg-white/10 backdrop-blur-sm rounded-xl border border-white/10 text-xs font-bold">
                               <Phone className="w-4 h-4 text-primary" /> {d.phone}
                             </div>
                           )}
                        </div>
                      </div>
                    ...
                    <div className="grid md:grid-cols-2 gap-8 px-2">
                      <div className="space-y-6">
                        ...
                        <div className="space-y-4">
                          {d.linkedin && <a href={d.linkedin} target="_blank" className="flex items-center gap-3 text-primary hover:underline font-bold transition-all text-sm"><Link className="w-4 h-4" /> Profil LinkedIn</a>}
                          {d.website && <a href={d.website} target="_blank" className="flex items-center gap-3 text-primary hover:underline font-bold transition-all text-sm"><Globe className="w-4 h-4" /> Portfolio / Site</a>}
                          {d.address && <div className="text-sm text-slate-600 font-medium">📍 {d.address}</div>}
                        </div>
                      </div>
                      <div className="space-y-6">
                        ...
                        <div className="text-slate-700 italic">
                          <FormattedText text={d.summary || "Aucun résumé disponible."} />
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-8">
                      ...
                      <div className="space-y-8">
                        {d.experiences?.map((exp: Record<string, string>, i: number) => (
                    ...
                    <div className="space-y-8">
                      ...
                      <div className="grid sm:grid-cols-2 gap-6">
                        {d.educations?.map((edu: Record<string, string>, i: number) => (
